...
# -*- coding: utf-8 -*-
"""
将 PlantUML Activity (Act) 与 Sequence (Seq) 两个数据集转换为 image-to-code (i2c) 检索格式。

功能要点：
- 支持 Act / Seq 两个数据集，分别处理其 train / test 子集。
- 同名 .png 与 .txt 文件视为一一配对；从 .txt 读取代码作为目标文本。
- 复制所有查询图片到统一目录：
...
示例用法：
  python PlantUML/convert_plantuml_i2c.py --limit-train 50 --limit-test 50 --overwrite-images
...
def build_train_rows(items: List[PairItem], out_images_dir_rel: str, prompt: str) -> List[Dict]:
    ...
    for it in items:
        try:
            codes.append(it.txt_path.read_text(encoding="utf-8"))
        except Exception:
            codes.append("")
    ...
    for idx, it in enumerate(items):
        ...
        pos_text = codes[idx]
        # 随机选择一个不同 index 作为负样本；若集合过小，则置空
        neg_text = ""
        if len(codes) > 1:
            j = random.randrange(len(codes))
            if j == idx:
                j = (j + 1) % len(codes)
            neg_text = codes[j]
...
def main():
    ...
    for subset in subsets:
        ...
        out_dir_train = TRAIN_ROOT / f"{ds_label}_i2c"
        out_dir_test = TEST_ROOT / f"{ds_label}_i2c"
        out_images_train = TRAIN_ROOT / "images" / ds_label / "images"
        out_images_test = TEST_ROOT / "images" / ds_label / "images"

        prompt = (
            "Please convert this image to PlantUML activity diagram code."
            if is_act
            else "Please convert this image to PlantUML sequence diagram code."
        )
