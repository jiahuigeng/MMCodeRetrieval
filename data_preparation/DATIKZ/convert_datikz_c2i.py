...
# -*- coding: utf-8 -*-
"""
将 nllg/datikz-v3 数据集转换为 MMCoIR c2i（code-to-image）训练/测试集。
...
输出：
- 写入 `MMCoIR-train/DATIKZ_c2i/train.jsonl` 与 `MMCoIR-test/DATIKZ_c2i/test.jsonl`（可通过 `--out-name` 自定义）。
- 图片分别复制到：
  - `MMCoIR-train/images/DATIKZ/images/`
  - `MMCoIR-test/images/DATIKZ/images/`
- JSONL 字段（4 个 key，与规范一致）：
  - `qry_text`: 包含代码的查询文本：`Please convert this code to image.\n<code>`
  - `qry_img_path`: 空字符串（本任务无查询图片）
  - `tgt_text`: 目标文本列表（包含一个 image token，例如 `<|image_1|>`）
  - `tgt_img_path`: 目标图片路径列表（相对路径 `images/DATIKZ/images/<filename>`）

示例：
  python DATIKZ/convert_datikz_c2i.py \
...
- `--image-token TOKEN`   目标文本图片 token（默认 `<|image_1|>`）
...
- `--out-name NAME`       JSONL 输出数据集文件夹名（默认 DATIKZ_c2i）
- `--images-name NAME`    图片目录与 tgt_img_path 的数据集名（默认 DATIKZ）
"""
    ...
    for fp in files:
        ...
        for _, row in df.iterrows():
            ...
            if img is None or code is None:
                continue
            # 保留原始图像值（可能是 str 路径、dict 包含 bytes/path、PIL.Image、或字节），代码转 str
...
def main():
    parser = argparse.ArgumentParser(description="Convert DATIKZ (nllg/datikz-v3) to MMCoIR c2i train/test JSONL with sampling")
    ...
    parser.add_argument("--image-token", type=str, default=DEFAULT_IMAGE_TOKEN, help="目标文本中的图片 token（默认 <|image_1|>）")
    ...
    parser.add_argument("--out-name", type=str, default="DATIKZ_c2i", help="JSONL 输出数据集文件夹名（默认 DATIKZ_c2i）")
    parser.add_argument("--images-name", type=str, default="DATIKZ", help="图片目录与路径前缀的数据集名（默认 DATIKZ）")
    ...
    # 复制目录目标（图片目录不带后缀，使用 images-name）
    ...
    print("[DONE] DATIKZ -> c2i 训练/测试集 完成")
