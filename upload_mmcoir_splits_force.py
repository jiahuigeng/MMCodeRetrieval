#!/usr/bin/env python3
"""
Force upload MMCoIR-train and MMCoIR-test splits to Hugging Face dataset repos.
- Upload JSONL and images.tar.gz per dataset subfolder.
- Always upload local README.md from split root (MMCoIR-*/README.md).
- DO NOT skip when remote subfolder/files already exist (if --force is set).

参考 upload_mmcoir_splits.py 的实现，增加强制覆盖选项以便上传特定文件夹的内容，
例如强制上传 MMCoIR-test/MMSVG-Illustration，即使远程目录已存在。

用法示例：
  python upload_mmcoir_splits_force.py --create-repo --check --force \
    --test-root MMCoIR-test --test-repo JiahuiGengNLP/MMCoIR-test \
    --dataset MMSVG-Illustration

  python upload_mmcoir_splits_force.py --force --dry-run \
    --train-root MMCoIR-train --train-repo JiahuiGengNLP/MMCoIR-train \
    --dataset ChartGen --dataset Chart2Code

鉴权：先运行 `huggingface-cli login` 或设置环境变量 HF_TOKEN。
"""
...
def upload_files(api: HfApi, repo_id: str, files: List[Tuple[str, Path]], prefix: str, commit_prefix: str, remote_files: List[str], dry_run: bool, force_upload: bool) -> None:
    """Upload files to repo. If force_upload is True, overwrite even if exists."""
    for basename, path_obj in files:
        local_path = str(path_obj)
        path_in_repo = f"{prefix}/{basename}"

        if path_in_repo in remote_files and not force_upload:
            print(f"[SKIP] 远端已存在: {path_in_repo}")
            continue
        elif path_in_repo in remote_files and force_upload:
            print(f"[FORCE] 覆盖上传: {path_in_repo}")
...
# --------------------------
# Main upload logic (force-aware)
# --------------------------
def process_split(api: HfApi, split_root: Path, repo_id: str, split: str, datasets: Optional[List[str]], images_tar_name: str, force_repack: bool, create_repo_flag: bool, check: bool, dry_run: bool, commit_prefix: str, force_upload: bool) -> None:
    ...
    for d in ds_dirs:
        ...
        # 远端是否已有该子目录
        has_remote_subdir = any(f.startswith(f"{name}/") for f in remote_files)
        if has_remote_subdir and not force_upload:
            print(f"[SKIP] 远端已存在子目录: {name}/，跳过整个上传。")
            continue
        elif has_remote_subdir and force_upload:
            print(f"[FORCE] 远端存在 {name}/，仍继续覆盖上传该子目录。")
        ...
        # 上传（强制覆盖时不跳过已存在文件）
        upload_files(api, repo_id, mapping, prefix=name, commit_prefix=commit_prefix, remote_files=remote_files, dry_run=dry_run, force_upload=force_upload)
...
# --------------------------
def main() -> None:
    ap = argparse.ArgumentParser(description="Force upload MMCoIR-train/test JSONL + images.tar.gz to HF (overwrite if exists)")
    ...
    ap.add_argument("--dataset", action="append", default=[], help="限定数据集子目录名，可重复 (如 MMSVG-Illustration, ChartGen)")
    ...
    ap.add_argument("--force", action="store_true", help="强制覆盖上传，即使远端目录/文件已存在")
    args = ap.parse_args()
