...
"""
Convert ServiceNow/BigDocs-Sketch2Flow into image-to-code (i2c) JSONL.

Project-required fields:
- Train JSONL: {"qry", "qry_image_path", "pos_text", "pos_image_path", "neg_text", "neg_image_path"}
- Test  JSONL: {"qry_text", "qry_img_path", "tgt_text", "tgt_img_path"}

Design for i2c:
- qry/qry_text: contains an image token "<|image_1|>" and instruction
  "Please convert this image to code.".
- qry_image_path/qry_img_path: relative path to the copied image file.
- pos_text/tgt_text: the target code from `annotations` (string or JSON-serialized).
- pos_image_path/tgt_img_path: empty ("" / [""] respectively), as target is code.
...
def collect_items(dset_split) -> List[Tuple[str, Any, str]]:
    ...
    for rec in dset_split:
        ...
        if not code:
            continue
        items.append((ident.strip(), imgs[0], code))
    return items
...
def build_train_rows(items: List[Tuple[str, str, str]]) -> List[Dict[str, Any]]:
    ...
    prompt = f"{IMG_TOKEN}\n{INSTR}"
    for ident, rel_img_path, code in items:
        rows.append(
            {
                "qry": prompt,
...
def main():
    parser = argparse.ArgumentParser(description="Convert Sketch2Flow to i2c JSONL and copy images.")
    parser.add_argument(
        "--data-root",
        type=str,
    ...
    def process_split(items: List[Tuple[str, Any, str]], img_dst_dir: Path) -> List[Tuple[str, str, str]]:
        result: List[Tuple[str, str, str]] = []
        for ident, img_entry, code in items:
            ...
            result.append((ident, rel_path, code))
        return result
