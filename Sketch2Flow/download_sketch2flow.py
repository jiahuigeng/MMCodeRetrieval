#!/usr/bin/env python3
"""
Download the ServiceNow/BigDocs-Sketch2Flow dataset from Hugging Face Hub.
...
Usage
  ...
  python Sketch2Flow/download_sketch2flow.py --output_dir datasets/Sketch2Flow

  # Specify repo id explicitly
  python Sketch2Flow/download_sketch2flow.py --repo_id ServiceNow/BigDocs-Sketch2Flow --output_dir datasets/Sketch2Flow

  # Filter only certain subsets (depends on dataset structure)
  python Sketch2Flow/download_sketch2flow.py --allow train/** --allow validation/** --output_dir datasets/Sketch2Flow

  # Ignore large or unnecessary paths
  python Sketch2Flow/download_sketch2flow.py --ignore examples/** --ignore docs/** --output_dir datasets/Sketch2Flow
...
def main():
    parser = argparse.ArgumentParser(description="Download ServiceNow/BigDocs-Sketch2Flow from Hugging Face Hub")
    parser.add_argument("--repo_id", type=str, default="ServiceNow/BigDocs-Sketch2Flow", help="HF dataset repo id")
    parser.add_argument("--output_dir", type=str, default="datasets/Sketch2Flow", help="Local output directory")
