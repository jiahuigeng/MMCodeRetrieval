...
def parse_args():
    parser = argparse.ArgumentParser(
        description="下载 Hugging Face 数据集 nllg/datikz-v2 到项目根目录的 datasets 目录"
    )
    parser.add_argument(
        "--repo-id",
        default="nllg/datikz-v2",
        help="Hugging Face 数据集仓库 ID，默认：nllg/datikz-v2",
    )
    # 默认下载到 <repo_root>/datasets/datikz-v2
    repo_root = Path(__file__).resolve().parent.parent
    default_outdir = repo_root / "datasets" / "datikz-v2"
    parser.add_argument(
        "--outdir",
