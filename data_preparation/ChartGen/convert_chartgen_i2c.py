...
"""
任务说明（i2c 版本）
- 模仿 convert_chartgen_split.py，将 datasets/ChartGen-200K 下的 Parquet 数据合并处理为图片到代码（image-to-code）检索格式。
- 随机采样：训练 100000、测试 2000（可通过参数调整）。
- 输出：
  - 训练集 JSONL -> `MMCoIR-train/ChartGen_i2c/train.jsonl`
  - 测试集 JSONL -> `MMCoIR-test/ChartGen_i2c/test.jsonl`
- 复制图片：
...
- JSONL 中图片路径统一为相对路径 `images/ChartGen/images/<filename>`，对应更新 `qry_image_path`/`qry_img_path`。
"""
...
TRAIN_ROOT_DEFAULT = REPO_ROOT / "MMCoIR-train" / "ChartGen_i2c"
TEST_ROOT_DEFAULT = REPO_ROOT / "MMCoIR-test" / "ChartGen_i2c"
...
def to_train_item(code: str, basename: str) -> dict:
    return {
        "qry": f"<|image_1|>\n{PROMPT}",
        "qry_image_path": f"images/ChartGen/images/{basename}",
        "pos_text": str(code),
        "pos_image_path": "",
        "neg_text": "",
        "neg_image_path": "",
...
def to_test_item(code: str, basename: str) -> dict:
    return {
        "qry_text": f"<|image_1|>\n{PROMPT}",
        "qry_img_path": f"images/ChartGen/images/{basename}",
        "tgt_text": [str(code)],
        "tgt_img_path": [""],
    }
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="ChartGen-200K image-to-code: split into train/test JSONL and copy images"
    )
    ...
    train_map = {b: c for (b, c) in train_candidates}
    test_map = {b: c for (b, c) in test_candidates}
    ...
    # Save JSONL (folder has _i2c suffix, files without)
    ...
    print("[DONE] ChartGen i2c conversion completed.")
